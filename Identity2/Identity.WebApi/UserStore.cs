...
namespace Identity.WebApi
{
    ...
    public class UserStore : UserStore<string, Role, User, UserClaim, UserLogin, UserRole, 
        ...
        IUserStore<User> 
    {
        public UserStore(IWebApiUOW uow)
            : base(uow)
        {
        }
    }
...

    {
        ...
        public Task UpdateAsync(TUser user)
        {
            ...
            _uow.GetRepository<TUserRepository>().Update(user.Id, user);
            _uow.Commit();
        ...
        public Task RemoveLoginAsync(TUser user, UserLoginInfo login)
        {
            ...
            var userLogin = _uow.GetRepository<TUserLoginRepository>().GetUserLoginByProviderAndProviderKey(login.LoginProvider, login.ProviderKey);
        ...
        public Task AddClaimAsync(TUser user, System.Security.Claims.Claim claim)
        {
            ...
            _uow.GetRepository<TUserClaimRepository>().Add(new TUserClaim() { ClaimValue = claim.Value, ClaimType = claim.Type, UserId = user.Id });
            _uow.Commit();
            return Task.FromResult<object>(null);
        }
        ...
        public Task RemoveClaimAsync(TUser user, System.Security.Claims.Claim claim)
        {
            ...
            var c =
                _uow.GetRepository<TUserClaimRepository>().All.FirstOrDefault(a => a.ClaimType == claim.Type && a.ClaimValue == claim.Value && a.UserId.Equals(user.Id));
            if (c != null)
            {
